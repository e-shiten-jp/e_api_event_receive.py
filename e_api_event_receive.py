...
# 引数1：銘柄コード string
# 引数2：市場 string
# 引数3: class_cust_property（口座属性クラス）
# 返値： url string
# 備考: NS（ニュース）は重いためサンプルでは除外。
def func_make_event_url(int_p_gyou_no, str_sIssueCode, str_sSizyouC, class_cust_property):
    str_url = ''
...
##    str_url = str_url + 'p_evt_cmd=ST,KP,EC,SS,US,FD'
    ...
    str_url = str_url + '&' + 'p_gyou_no=' + str(int_p_gyou_no)      # 行番号。1-120で指定。
